...
export const crearLote = async (req, res) => {
  try {
    ...
    const loteConDatos = await Lote.findById(lote._id).populate(
      "productos",
      "nombreProducto"
    );
...
export const listarLotes = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      ...
      lotes,
      message:
        lotes.length === 0 ? "No se encontraron lotes activos" : undefined,
    });
  } catch (err) {
    return res.status(500).json({
      success: false,
...
export const actualizarLote = async (req, res) => {
  try {
    ...
    const lote = await Lote.findByIdAndUpdate(id, data, { new: true }).populate(
      "productos",
      "nombreProducto"
    );
...
export const listarTotalProductos = async (req, res) => {
  try {
    ...
    const lotes = await Lote.find({
      "productos.productoId": producto._id,
      estado: true,
    });

    const totalProducto = lotes.reduce(
    ...
    const filePath = path.join(
      uploadsDir,
      `${producto.nombreProducto}_reporte.pdf`
    );
...
export const generarPDFLotes = async (req, res) => {
  try {
    ...
    lotes.forEach((lote) => {
      const nombresProductos = lote.productos
        .map((p) => p.productoId?.nombreProducto || "Sin nombre")
        .join(", ");
...
export const sumarPrecioTotalLotes = async (req, res) => {
  try {
    const lotes = await Lote.find({ estado: true }).populate(
      "productos",
      "precio nombreProducto"
    );

    const precioTotal = lotes.reduce((total, lote) => {
      const cantidad = parseInt(lote.cantidad, 10) || 0;
      const precio = lote.productos?.precio || 0;
      return total + cantidad * precio;
    }, 0);

    return res.status(200).json({
      success: true,
      message: "Suma total de precios calculada exitosamente",
      precioTotal,
    });
  } catch (error) {
    return res.status(500).json({
      success: false,
      message: "Error al calcular la suma total de precios",
      error: error.message,
    });
  }
};
